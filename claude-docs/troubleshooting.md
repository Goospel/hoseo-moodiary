...
>
> 마지막 갱신: 2026-05-28 (T-029 추가 — MkDocs `--strict` 빌드 시 저장소 `README.md` 와 사이트 `index.md` 가 자동 충돌 (MkDocs 가 README 를 index 후보로 가정). 사이트 안에 둘 다 노출하려면 README 를 다른 파일명 (예: `project-readme.md`) 으로 복사하고 nav / 링크도 그 이름으로)
